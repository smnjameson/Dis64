...
const Enhance = {
    ...
    DetermineEnvironment(sourceCode) {
    ...
    GenerateLabels(sourceCode) {
        ...
        for(var i=0; i<sourceCode.length; i++) {
            ...
            if(op.target) {
                let found = false;
                //COLRAM
                if (op.opcode !== "jsr" && op.opcode !== "jmp" && op.mode !== "rel" && op.target >= 0xd800 && op.target < 0xd800 + 0x3e8) {
                    op.src = op.src.replace(/\$[0-9a-fA-F]{1,4}/, SysLabels["d800"] + " + $" + (op.target - 0xd800).toString(16).padStart(2,"0"))
                    found = true
                } else if(Enhance.SCREEN_BUFFERS.length) {
                    //SCREEN BUFFERS
                    for(var j=0; j< Enhance.SCREEN_BUFFERS.length; j++) {
                        if (op.opcode !== "jsr" && op.opcode !== "jmp" && op.mode !== "rel" && op.target >= Enhance.SCREEN_BUFFERS[j] && op.target < Enhance.SCREEN_BUFFERS[j] + 0x3e8) {
